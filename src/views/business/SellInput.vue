<template>
  <div class="SellInput">
    <top-head title="Jual" @leftClick="goBack"></top-head>

    <div class="sell_title">Jumlah jual</div>
    ...
    <div class="sell_item price">
      <input
        type="number"
        placeholder="Silahkan masukan harga jual"
        v-model.number="price"
        @input="priceInput"
      >
      <span class="right">Rpc</span>
    </div>
    <!-- <div class="subTit">Minimum jual dengan harga {{min_price}} Rp coin</div> -->
    ...
    <div class="submit" :class="{ active: canSubmit }" @click="submitClick">Setuju</div>
...
export default {
  ...
  mounted() {
    ...
    if (config && config.saleConf) {
      this.placeholder = "Jumlah minimum (" + config.saleConf.min_num + ")";
    }
  },
  components: {
    TopHead
  ...
  computed: {
    ...
    min_price() {
      var config = this.$store.state.business.businessConfig;
      if (config && config.saleConf) {
        return parseFloat(config.saleConf.min_price);
      } else {
        return 0.1;
      }
    },
    min_advice() {
      var config = this.$store.state.business.businessConfig;
      if (config && config.saleConf) {
        return parseFloat(config.saleConf.fast_area_min);
      } else {
        return 0.1;
      }
    },
    max_advice() {
      var config = this.$store.state.business.businessConfig;
      if (config && config.saleConf) {
        return parseFloat(config.saleConf.fast_area_max);
      } else {
        return 0.1;
      }
    },
    max_price() {
      var config = this.$store.state.business.businessConfig;
      if (config && config.saleConf) {
        return parseFloat(config.saleConf.max_price);
      } else {
        return 9999999;
      }
    },
      ...
      if (this.canSubmit) {
        ...
        if (this.value < this.min) {
          ...
          if (
            config &&
            config.saleConf &&
            parseFloat(config.saleConf.fast_area_max) &&
            parseFloat(this.price) > parseFloat(config.saleConf.fast_area_max)
          ) {
            // 大于指导价提示
            Dialog.confirm({
