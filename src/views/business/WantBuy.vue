<template>
  <div class="WantBuy">
    <div class="top_msg">Beli Hi-Koin, jika tidak menemukan harga yang tepat, maka klik tombil di bawah ini</div>
    <div class="buylist">
      <van-pull-refresh v-model="loading" @refresh="refresh">
        ...
        >
          <div v-for="item in list" v-show="list.length" class="list_item" :key="item.id">
            <div class="gobuy" @click="goBuy(item)">Beli</div>
            <div class="num">Jumlah：{{item.sale_num|numFormatD}} Hi-Koin</div>
            <div class="price">Harga：Rpc{{item.price|numFormatD}}</div>
            <div class="name">
              <span>{{item.username}}</span>
            </div>
            <div class="credit">Jumlah Transaksi：{{item.trans_sale_num}}</div>
          </div>
        </van-list>
    ...
    <van-popup v-model="show_buy" position="bottom" @close="closePop">
      <div class="buyBox">
        ...
        <div class="input_box">
          ...
          <div class="all_btn" @click="buyAllClick">Beli semua</div>
        </div>
        <div class="describe">Pembelian minimum {{show_min}} Hi-Koin</div>
        <div class="box_title">
        ...
        <div class="btn_box">
          ...
          <span class="buy_btn" @click="buySure">Beli</span>
        </div>
      </div>
    </van-popup>
...
<script>
import { getBusinessBuyList, goBuyHiBean } from "utils/ajax";
...
export default {
  ...
  computed: {
    ...
    min_num() {
      ...
      if (config && config.buyConf) {
        return parseFloat(config.buyConf.min_num);
      } else {
        return 0.1;
      }
    }
  ...
  methods: {
    goBuy(item) {
      ...
      if (parseFloat(item.sale_num) < parseFloat(this.min_num)) {
        this.buy_value = parseFloat(item.sale_num);
        this.show_min = parseFloat(item.sale_num);
        this.disabled = true;
      } else {
    ...
    buyAllClick() {
      this.buy_value = parseFloat(this.buy_item.sale_num);
    },
    buySure() {
      if (!this.buy_value) {
        //购买数量小于最低购买数量
      ...
      } else if (this.buy_value > parseFloat(this.buy_item.sale_num)) {
        // 购买数量大于挂单数量
        Toast("Jumlah Hi-Koin tidak boleh melebihi jumlah jual");
      } else {
        let param = {
          sale_order_id: this.buy_item.id,
          buy_num: this.buy_value
        };
    ...
    goInput() {
      this.$router.push("/recycle-input");
    },
